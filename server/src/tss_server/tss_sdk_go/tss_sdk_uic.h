...
typedef struct
{
    TssSdkOpenid openid_;                   /* [in] 用户openid */
    unsigned char plat_id_;                 /* [in] plat_id, 0: IOS, 1: Android */
    UicMsgResultFlag msg_result_flag_;      /* [in] 用户输入经过sdk的判定结果，开发方根据结果决定是否屏蔽 User input after the verdict of the sdk, developer to decide whether to shield in accordance with the results */
...
typedef struct
{
    ...
    char if_replace_;                       /* [in] 如果包含敏感词，是否替换成“*”。如果是，填1；如果否，填0 If it contains sensitive words, whether to replace with the "*". If yes, fill in 1; if not, fill in 0*/
