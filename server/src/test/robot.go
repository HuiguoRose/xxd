...
func (this *Robot) Login(id int) {
	...
	this.gs.Player_FromPlatformLogin(
		player_api.PLATFORM_TYPE_MOBILE_IOS_GUEST, // platform_type,
		this.name, // username,
		this.name, // nickname,
		MAIN_ROLE, // role_id,
...
func main() {
	...
	for i := 1; i <= int(robotNum); i++ {
		go func(id int, robot *Robot) {
			...
			robot.ActOnce(ActOnce_ReciveWorldChannelBroadcast, true, 1)
			robot.Action(Action_WorldChannelChat, false, 5)
		}(i, NewRobot())
	}
...
func ActOnce_ReciveWorldChannelBroadcast(client *client_test.Client, robot *Robot) {
	//fmt.Printf("set global broadcast\n")
	var count uint64
	robot.hd.OutNotify_SendGlobalChats = func(out *notify_api.SendGlobalChats_Out) {
		if robot.id == 5 {
			count += uint64(len(out.Chats))
			fmt.Printf("%v recive %d messages from world channel\n", time.Now(), count)
		}
	}
}

func Action_WorldChannelChat(client *client_test.Client, robot *Robot) {
	client.Debug_AddWorldChatMessage(2)
}
