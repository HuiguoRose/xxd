...
// Verify sends receipts and gets validation result
func (c *Client) Verify6(req IAPRequest) (IAPResponse6, []byte, error) {
	result := IAPResponse6{}
	res, body, errs := gorequest.New().
		Post(c.URL).
		Send(req).
	...
	if errs != nil {
		return result, nil, fmt.Errorf("%v", errs)
	}
	if res.StatusCode < 200 || res.StatusCode >= 300 {
		return result, nil, errors.New("An error occurred in IAP - code:" + strconv.Itoa(res.StatusCode))
	}
